...
spill_length = 20000000
# spill_length = 55406920
error_length =  3000000
...
class ScalerAnalyzer():
  ...
  def __init__(self, port=9096):
    ...
    self.server.SetJSROOT('https://root.cern.ch/js/7.9.2/')
 ...
 #____________________________________________________________________________
  ...
  def print_scaler_sheet(self, file_name, lpr=True):
    ...
    self.draw_one_line(16, ['L1Req/Beam', 'L2-Eff', 'Duty-Factor'],
                       footer=True)
    output_path = os.path.join(scaler_dir, file_name)
