...
process_begin( const std::vector<std::string>& argv )
{
  ...
  // Hodoscopes
  int nbinsqdc=512;
  gHttp.Register(gHist.createHodo(kBHD,"BHD",16,2,nbinsqdc,-0.5,2047.5,4000,0,4000));
  gHttp.Register(gHist.createHodo(kT0,"T0",5,2,nbinsqdc,-0.5,4095.5,1e4,2e6,4e6));
  gHttp.Register(gHist.createHodo(kT0new,"T0new",1,2,nbinsqdc,-0.5,4095.5,1e4,2e6,4e6));
  gHttp.Register(gHist.createHodo(kAC ,"AC",5,1,nbinsqdc,0,2048,1e4,0,1e7));
  gHttp.Register(gHist.createHodo(kE0,"E0",3,2,nbinsqdc,-0.5,4095.5,1e4,0,1e7));
  gHttp.Register(gHist.createHodo(kDEF,"DEF",5,2,nbinsqdc,-0.5,4095.5,1e4,0,1e7));
  gHttp.Register(gHist.createHodo(kCDH,"CDH",36,2,nbinsqdc,-0.5,4095.5,1e4,0,1e7));
  gHttp.Register(gHist.createMHTDC(kTriggerFlag,"TriggerFlag",16,10000));
  gHttp.Register(gHist.createSDD(kSDD ,"SDD",12,4));
  // Chambers
  ...
  gHttp.Register(http::QDC(kE0,"_E0_U",0,0,3,3,1,0,1000),"E0");
  gHttp.Register(http::QDC(kE0,"_E0_D",1,0,3,3,1,0,1000),"E0");
  gHttp.Register(http::MHTDCTDC(kE0,"_E0_U",0,3,3,1),"E0");
  gHttp.Register(http::MHTDCTDC(kE0,"_E0_D",1,3,3,1),"E0");
  //gHttp.Register(http::MHTDCMeanTime(kE0,"_E0",0,3,3,1),"E0");
  gHttp.Register(http::MHTDCHitPatMulti(kE0,"_E0"),"E0");
  ...
  //gHttp.Register(http::MHTDCMeanTime(kCDH,"_CDH",0,36,6,6),"CDH");
  gHttp.Register(http::MHTDCHitPatMulti(kCDH,"_CDH"),"CDH");

  // SDD
  for(int i=0;i<12;i++){
    gHttp.Register(http::SDDADC(kSDD,Form("_SDD_%d",i),i,0,4000),"SDD");
  }
  gHttp.Register(http::SDDMHTDC(kSDD,"_SDDgate" ,0,48,8,6),"SDD");
  gHttp.Register(http::SDDMHTDC(kSDD,"_SDDreset",1,48,8,6),"SDD");
  ...
  std::ifstream data("/home/oper/e62ana/online/CDCmap.txt");
  std::string str;
  if (data.fail()) {
...
process_event( void )
{
  ...
  int hid;
  {
    // data type
    const int nhodo=6;
    DetectorType hodo[nhodo]={kBHD,kT0,kT0new,kE0,kDEF,kCDH};
    TString hodo_name[nhodo]={"BHD","T0","T0new","E0","DEF","CDH"};
    const int nsegs[nhodo]={16,5,1,3,5,36};
    const int nud[nhodo]={2,2,2,2,2,2};
    const int k_adc=0;
  ...
  // Aerogel 
  {
	...
	if(tdc<2.1e6&&tdc>2e6) ACHIT=true;
	hptr_array[hid]->Fill(tdc);	    
      }
    }
