...
namespace analyzer
{
  ...
  namespace
  {
    ...
    const int nchm=7;
    int ChmCid[nchm]={DetIdBLC1a,DetIdBLC1b,DetIdBLC2a,DetIdBLC2b,DetIdBPC1,DetIdBPC2,DetIdCDC};
    ...
    const int nwires[nchm]={32,32,32,32,15,32,16};
    const int nlayers[nchm]={8,8,8,8,8,8,118};
    double rotz[nchm]={45,45,135,135,-45,0,0};
    double ffz[nchm]={0,0,130,130,50,37,0};
...
{  
  ...
  gHttp.Register(gHist.createHodo(kVeto,"Veto",4,2,1024,0,1024,tdcnbins,tdcmin,tdcmax));
  gHttp.Register(gHist.createHodo(kBTC,"BTC",2,2,1024,0,1024,tdcnbins,tdcmin,tdcmax));
  gHttp.Register(gHist.createHodo(kPbF2,"PbF2",36,1,2048,0,2048,tdcnbins,tdcmin,tdcmax));
  gHttp.Register(gHist.createHodo(kPbG,"PbG",40,1,2048,0,2048,tdcnbins,tdcmin,tdcmax));
  ...
  gHttp.Register(http::BLDCMulti(kCDC2,"_CDC",15,0,5,3), "Multiplicity");
...
process_event( void )
{
...

  {
    // data type
    const int nhodo=9;
    int Cid[nhodo]={DetIdT1,DetIdBHT,DetIdT0,DetIdDEF,DetIdVeto,DetIdBTC,DetIdPbF2,DetIdPbG,DetIdCDH}; 
    DetectorType hodo[nhodo]={kT1, kBHT, kT0, kDEF, kVeto,kBTC,kPbF2,kPbG,kCDH};
    // TString hodo_name[nhodo]={"T1","BHT","T0","DEF","Veto","BTC","PbF2","PbG","CDH"};
    // const int nsegs[nhodo]={1,63,5,5,4,2,36,40,36};
    ...
    for(int ihodo=0;ihodo<nhodo;ihodo++){
      ...
      if(detid==DetIdCDH){
        tdc_ll=gUser.GetParameter("CDHTDC",0);
        tdc_ul=gUser.GetParameter("CDHTDC",1);
      }
      for( int i=0; i<nh; ++i ){
	...
	for(int it=0;it<hit->GetIndex();it++){
	  ...
	  if(ihodo!=0&&time0>-9000){
	    ...
	    if(kDET==kVeto) tof_veto.push_back(tof);
  ...
  // CDC
  {
    ...
    for( int i=0; i<15; i++ ){
      ...
      if(TRIG[7]){
        hid=gHist.getSequentialID(kDET2,0,kMulti2D,i+1);
        hptr_array[hid]->Fill(mul_gate[i]);
      }
    }
  }
