...
namespace analyzer
{
  ...
  namespace
  {
    ...
    // hodoscopes
    const int nhodo=9;
    DetectorType hodo[nhodo]={kT0,kT0new,kDEF,kCDH,kVeto,kPbF2,kPbG,kRC,kBTC};
    TString hodo_name[nhodo]={"T0","T0new","DEF","CDH","Veto","PbF2","PbG","RC","BTC"};
    const int nsegs[nhodo]={5,1,5,36,4,36,40,8,2};
    const int nud[nhodo]={2,2,2,2,2,1,1,2,2};
    const int k_adc=0;
...
process_begin( const std::vector<std::string>& argv )
{
  ...
  gHttp.Register(gHist.createHodo(kPbF2		,"PbF2" ,36, 1, nbinsqdc,-0.5,2047.5,nbinshrtdc,hrtdcmin,hrtdcmax)); 
  gHttp.Register(gHist.createHodo(kPbG		,"PbG"  ,40, 1, nbinsqdc,-0.5,2047.5,nbinshrtdc,hrtdcmin,hrtdcmax)); 
  gHttp.Register(gHist.createHodo(kBTC		,"BTC"  , 2, 2, nbinsqdc,-0.5,2047.5,nbinshrtdc,hrtdcmin,hrtdcmax)); 
