...
process_event()
{
  ...
  ++event_count;
  if(event_count%1000!=0) return 0;
  std::cout << "\033[2J" << std::endl;
  int event_number = gUnpacker.get_event_number();
  int run_number = gUnpacker.get_root()->get_run_number();
  ...
  }
  {
    ...
    int evnum=-1;
    int plane=112;
    int nhit = gUnpacker.get_entries(k_device, plane, 0, 0, 4);
    if(nhit==1){
      spill = gUnpacker.get(k_device, plane, 0, 0, 4);
    }
    nhit = gUnpacker.get_entries(k_device, plane, 0, 0, 3);
    if(nhit==1){
      evnum = gUnpacker.get(k_device, plane, 0, 0, 3);
    }
    std::cout<<std::setw(10)<<name
	     <<std::setw(15)<<""<<" : "
	     <<std::setw(10)<<"spill"
