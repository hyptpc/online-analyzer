...
process_begin( const std::vector<std::string>& argv )
{
  ...
  gHttp.Register(http::BH2HTOFPAD());
...
// #endif
  ...
  { ///// BH2
	...
	if(hit_flag[seg][ud] == 1){
	  ...
	  ++multiplicity;
	  hptr_array[hit_hid]->Fill(seg);
	  hptr_array[hit_hid+1]->SetBinContent( seg+1, 170 );
	  hptr_array[hit_hid+2]->SetBinContent( seg+1, hptr_array[hit_hid+2]->GetBinContent(seg+1)+1 );
  ...
  { ///// HTOF
	...
	hptr_array[htofhit_id+1]->SetBinContent( binid, 170 );
	hptr_array[htofhit_id+2]->SetBinContent( binid, hptr_array[htofhit_id+2]->GetBinContent(binid)+1);
	++multiplicity;
      }
    }//seg
...
//				     2193, 2413, 2634, 2858};
	...
	for( Int_t ch=0; ch<272; ++ch ){
	  ...
	  if( nhit != NumOfTimeBucket ){
	    ...
	    //hptr_array[tpca2d_id+4]->SetBinContent( pad + 1, 0 );
	    continue;
	  }
	  ...
	  // if( loc_max < 20 || 150 < loc_max ) continue;
	  ...
	  //if(max_adc-mean>110 && rms > 50)
	  if( loc_max > 20 && 150 > loc_max ){
	    hptr_array[tpca2d_id+4]->SetBinContent( pad + 1, loc_max );
	    n_active_pad_cut++;
	  }
	...
	for( Int_t i=0; i<NumOfTimeBucket; ++i ){
      ...
      static TCanvas c("TPC_TDC2D_TBCUT", "c", 1000, 1200);
      if(n_active_pad_cut > 100 && n_active_pad < 1000){
	hptr_array[tpca2d_id+4]->SetMaximum(170);
	hptr_array[tpca2d_id+4]->Draw("colz");
	c.SaveAs(Form("/misc/share/evdisp/run%05d-ev%d.png",
