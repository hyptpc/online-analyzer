...
process_begin( const std::vector<std::string>& argv )
{
  ...
  gHttp.Register(http::TOF(1),"Analysis");
  gHttp.Register(http::AC(),"Analysis");
  ...
  gHttp.Register(http::QDC(kBHD,"_BHD_U",0,0,16,4,4,0,1000),"BHD");
  gHttp.Register(http::QDC(kBHD,"_BHD_D",1,0,16,4,4,0,1000),"BHD");
  ...
  gHttp.Register(http::QDC(kE0,"_E0_U",0,0,3,2,2,0,1000),"E0");
  gHttp.Register(http::QDC(kE0,"_E0_D",1,0,3,2,2,0,1000),"E0");
  gHttp.Register(http::MHTDCTDC(kE0,"_E0_U",0,3,2,2),"E0");
  gHttp.Register(http::MHTDCTDC(kE0,"_E0_D",1,3,2,2),"E0");

  gHttp.Register(http::QDC(kDEF,"_DEF_U",0,0,5,3,2,0,4000),"DEF");
  gHttp.Register(http::QDC(kDEF,"_DEF_D",1,0,5,3,2,0,4000),"DEF");
...
process_event( void )
{
  ...
  double DEFADC[2]={-1,-1};
  {
    // data type
    const int nhodo=5;
    int Cid[nhodo]={DetIdT0new,DetIdBHD,DetIdT0,DetIdE0,DetIdDEF}; 
    DetectorType hodo[nhodo]={kT0new, kBHD, kT0, kE0, kDEF};
    TString hodo_name[nhodo]={"T0new","BHD","T0","E0","DEF"};
    const int nsegs[nhodo]={1,16,5,3,5};
    const int nud[nhodo]={2,2,2,2,2};
    const int k_adc=0;
	...
	for(int ud=0; ud<nud[i]; ++ud){	  
	  for(int idata=0; idata<ndata; ++idata){
		...
		switch(hodo[i]){
		case kBHD:
		  if(tdc>1900&&tdc<2100){
		    beamtiming[ud]=true;
		  }
		  break;
		default:
		  if(tdc>2.4e6&&tdc<2.6e6){
		    beamtiming[ud]=true;
		  }
		}
	      }
	...
	for( int m=0; m<nhit; ++m ){
	  ...
	  if(tdc<2.02e6&&tdc>1.96e6) ACHIT=true;
	  hptr_array[hid]->Fill(tdc);	    
	...
	for(int it=0;it<hit->GetIndex();it++){
	  ...
	  if(kDET==kT0new&&mt>10&&mt<30) time0=mt;
	  double tof=time0-mt;
	  ...
	  if(kDET==kBHD){
	    ...
	    else if(tof<35&&tof>30){
	      PION=true;
	    }
	  }	    
	  if(ihodo!=0&&time0>-9000){
	    hid  = gHist.getSequentialID(kAna, 0, 1, 0)+(ihodo-1)*4; 
	    hptr_array[hid]->Fill(tof);
	    // std::cout<<ihodo<<"  "<<hptr_array[hid]->GetTitle()
	    // 	     <<"  "<<hptr_array[hid+1]->GetTitle()
	    // 	     <<"  "<<hptr_array[hid+2]->GetTitle()
	    // 	     <<"  "<<hptr_array[hid+3]->GetTitle()<<std::endl;
  ...
  // Aerogel 
  {
    ...
    hid  = gHist.getSequentialID(kAna, 0, 3, 1);
    hptr_array[hid]->Fill(ac_sum);	    
    if(ACHIT){
    ...
    if(KAON)  hptr_array[hid+2]->Fill(ac_sum);	        
    if(PION)  hptr_array[hid+3]->Fill(ac_sum);	        
  }
  ...
  double rotz[nchm]={45,45,-45,-45,0};
  ...
  
  {
	...
	int mul=cont.size();
	//	if(kDET==kBPC) std::cout<<ichm<<"  "<<layer<<"  "<<mul<<std::endl;
	if(mul!=1) Single=false;
      }
      LocalTrack *track=new LocalTrack();
      if(Single&&TRIG[5]&&DEFADC[0]>500&&DEFADC[1]>500){
	//	if(kDET==kBPC)      std::cout<<"-----------"<<std::endl;
	for( int layer=1; layer<=nl; ++layer ){
	  const DCHitContainer &cont = DCAna->GetDCHC(cid, layer);
	  int mul=cont.size();
	  for(int ihit=0;ihit<mul;ihit++){
	    ...
	    if(mul==1){
	      ...
	      // if(kDET==kBPC){
	      // 	std::cout<<layer<<"  "<<hit->GetWirePosition()<<std::endl;
	      // }
	    }
	  }
	}
	...
	hid=gHist.getSequentialID(kDET,0,kProf,31);
	hptr_array[hid]->Fill(dxdz,dydz);
	hid=gHist.getSequentialID(kDET,0,kProf,32);
	hptr_array[hid]->Fill(x,dxdz);
	hid=gHist.getSequentialID(kDET,0,kProf,33);
	hptr_array[hid]->Fill(y,dydz);
	if(KAON){
